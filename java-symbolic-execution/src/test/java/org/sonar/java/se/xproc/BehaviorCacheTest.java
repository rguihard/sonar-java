...
class BehaviorCacheTest {
  ...
  void hardcoded_behaviors() throws Exception {
    ...
    assertThat(behaviorCache.hardcodedBehaviors()).hasSize(236);
    assertThat(logTester.logs(Level.DEBUG)).containsOnly("[SE] Loaded 236 hardcoded method behaviors.");
  }
    ...
    SECheck check = new SECheck() {
      ...
      public ProgramState checkPreStatement(CheckerContext context, Tree syntaxNode) {
        if (syntaxNode.is(Tree.Kind.METHOD_INVOCATION)) {
          Symbol.MethodSymbol symbol = ((MethodInvocationTree) syntaxNode).methodSymbol();
